...
### =============================================================
### $Id: admin/index.php,v 1.0 2008/01/03 19:00:00 GibaPhp Exp $
### =============================================================
...
if ( $op == "list" ) {
	...
	if ( is_array($polls_arr) && $polls_count > 0) {
                    ...
                    <tr>
                       <td class='bg2'>
		                  <table width='100%' border='0' cellpadding='4' cellspacing='1'>
		                     <tr class='bg3'>
                                <td>"._AM_DISPLAYBLOCK."</td>
                                <td>"._AM_DISPLAY_ID."</td>
                                <td>"._AM_DISPLAYORDER."</td>
                                <td>"._AM_POLLQUESTION."</td>
                                <td>"._AM_VOTERS."</td>
                                <td>"._AM_VOTES."</td>
                                <td>"._AM_EXPIRATION."</td>
                                <td>&nbsp;</td>
                             </tr>";
		...
		for ( $i = 0; $i < $max; $i++ ) {
			...
			echo "<tr class='bg1'>
                     <td align='center'>
                        <input type='hidden' name='poll_id[$i]' value='".$polls_arr[$i]->getVar("poll_id")."' />
                        <input type='hidden' name='old_display[$i]' value='".$polls_arr[$i]->getVar("display")."' />
                        <input type='checkbox' name='display[$i]' value='1'".$checked." />
                     </td>
                     <td>".$polls_arr[$i]->getVar("poll_id")."</td>
                     <td>
                        <input type='hidden' name='old_weight[$i]' value='".$polls_arr[$i]->getVar("weight")."' />
                        <input type='text' name='weight[$i]' value='".$polls_arr[$i]->getVar("weight")."' size='6' maxlength='5' />
                     </td>
                     <td>".$polls_arr[$i]->getVar("question")."</td>
                     <td align='center'>".$polls_arr[$i]->getVar("voters")."</td>
                     <td align='center'>".$polls_arr[$i]->getVar("votes")."</td>
                     <td>".$end."</td>
                     <td align='right'><a href='index.php?op=edit&amp;poll_id=".$polls_arr[$i]->getVar("poll_id")."'>"._EDIT."</a><br /><a href='index.php?op=delete&amp;poll_id=".$polls_arr[$i]->getVar("poll_id")."'>"._DELETE."</a><br /><a href='index.php?op=log&amp;poll_id=".$polls_arr[$i]->getVar("poll_id")."'>"._AM_VIEWLOG."</a></td>
                  </tr>";
		}
...
if ( $op == "add" ) {
	...
	$barcolor_array = XoopsLists::getImgListAsArray(XOOPS_ROOT_PATH."/modules/".$mydirname."/images/colorbars/");
	for($i = 0; $i < 10; $i++){
		...
		// claudia alterou
		$option_text = new XoopsFormDhtmlTextArea("", "option_text[".$i."]" ,'',2,50);		
		...
		$color_select->setExtra("onchange='showImgSelected(\"option_color_image[".$i."]\", \"option_color[".$i."]\", \"modules/".$mydirname."/images/colorbars\", \"\", \"".XOOPS_URL."\")'");
		$color_label = new XoopsFormLabel("", "<img src='".XOOPS_URL."/modules/".$mydirname."/images/colorbars/".$current_bar."' name='option_color_image[".$i."]' id='option_color_image[".$i."]' width='30' align='bottom' height='15' alt='' /><br />");
...
if ( $op == "edit" ) {
	...
	$poll_form->addElement($multi_yn);
	$options_arr =& xicpollOption::getAllByPollId($poll->getVar("poll_id"));
	$notify_value = 1;
	...
	foreach($options_arr as $option){
		...
		// claudia alterou 
		$option_text = new XoopsFormDhtmlTextArea("", "option_text[$i]" ,$option->getVar("option_text",'edit'),2,50);
		$option_tray->addElement($option_text);
		...
		$color_select->addOptionArray($barcolor_array);
		$color_select->setExtra("onchange='showImgSelected(\"option_color_image[".$i."]\", \"option_color[".$i."]\", \"modules/".$mydirname."/images/colorbars\", \"\", \"".XOOPS_URL."\")'");
		$color_label = new XoopsFormLabel("", "<img src='".XOOPS_URL."/modules/".$mydirname."/images/colorbars/".$option->getVar("option_color", "E")."' name='option_color_image[".$i."]' id='option_color_image[".$i."]' width='30' align='bottom' height='15' alt='' /><br />");
		$option_tray->addElement($color_select);
		$option_tray->addElement($color_label);
	...
	foreach ( $option_id as $opid ) {
		...
		if ( $option_text[$i] != "" ) {
			...
			if ( $option->delete() != false ) {
				xicpollLog::deleteByOptionId($option->getVar("option_id"));
			}
		}
		$i++;
	}
...
if ( $op == "delete_ok" ) {
	...
	if ( $poll->delete() != false ) {
		xicpollOption::deleteByPollId($poll->getVar("poll_id"));
		xicpollLog::deleteByPollId($poll->getVar("poll_id"));
		include_once XOOPS_ROOT_PATH.'/class/template.php';
		xoops_template_clear_module_cache($xoopsModule->getVar('mid'));
...
if ( $op == "restart_ok" ) {
	...
	if ( $reset == 1 ) {
		// reset all logs
		xicpollLog::deleteByPollId($poll->getVar("poll_id"));
		xicpollOption::resetCountByPollId($poll->getVar("poll_id"));
	}
	if (!$poll->store()) {
		echo $poll->getHtmlErrors();
		exit();
...
if ( $op == "log" ) {
	...
	// show logs
	$logs_arr =& xicpollLog::getAllByPollId($poll_id, $orderby." ".$orderdir);
	$logs_count = count($logs_arr);
	$arrow_up = XOOPS_URL."/modules/".$mydirname."/images/up.gif";
	$arrow_down = XOOPS_URL."/modules/".$mydirname."/images/down.gif";
...
//		echo "<td nowrap>&nbsp;</td>";
		...
		for ( $i = 0; $i < $max; $i++ ) {
			$option = new xicpollOption($logs_arr[$i]->getVar("option_id"));
			echo "<tr class='bg1'>";
