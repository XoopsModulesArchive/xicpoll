...
### =============================================================
### $Id: xicpoll.php,v 1.0 2008/01/03 19:00:00 GibaPhp Exp $
### =============================================================
...
class xicpoll extends XoopsObject
{
	var $db;

	//constructor
	function xicpoll($id=null)
	{
	...
	function store()
	{
		...
		if ( empty($poll_id) ) {
			$poll_id = $this->db->genId($this->db->prefix("xicpoll_desc")."_poll_id_seq");
			$sql = "INSERT INTO ".$this->db->prefix("xicpoll_desc")." (poll_id, 
			                                                             question, 
																		 description, 
																		 user_id, 
																		 start_time, 
			...
			$sql ="UPDATE ".$this->db->prefix("xicpoll_desc")." 
			      SET question=".$this->db->quoteString($question).", 
	...
	function load($id)
	{
		$sql = "SELECT * FROM ".$this->db->prefix("xicpoll_desc")." WHERE poll_id=".$id."";
	...
	function delete()
	{
		$sql = sprintf("DELETE FROM %s WHERE poll_id = %u", $this->db->prefix("xicpoll_desc"), $this->getVar("poll_id"));
	...
	function &getAll($criteria=array(), $asobject=true, $orderby="end_time DESC", $limit=0, $start=0)
	{
		...
		if ( !$asobject ) {
			$sql = "SELECT poll_id FROM ".$db->prefix("xicpoll_desc")."$where_query ORDER BY $orderby";
			$result = $db->query($sql,intval($limit),intval($start));
			while ( $myrow = $db->fetchArray($result) ) {
				$ret[] = $myrow['poll_id'];
			}
		} else {
			$sql = "SELECT * FROM ".$db->prefix("xicpoll_desc")."".$where_query." ORDER BY $orderby";
			$result = $db->query($sql,$limit,$start);
			while ( $myrow = $db->fetchArray($result) ) {
				$ret[] = new xicpoll($myrow);
			}
		}
		//echo $sql;
		return $ret;
	...
	function vote($option_id, $ip, $user_id=null)
	{
		if (!empty($option_id)) {
			if (is_array($option_id)) {
				...
				$option = new xicpollOption($option_id);
				if ( $this->getVar("poll_id") == $option->getVar("poll_id") ) {
					$log = new xicpollLog();
					$log->setVar("poll_id", $this->getVar("poll_id"));
	...
	{
		$votes = xicpollLog::getTotalVotesByPollId($this->getVar("poll_id"));
		$voters = xicpollLog::getTotalVotersByPollId($this->getVar("poll_id"));
		$sql ="UPDATE ".$this->db->prefix("xicpoll_desc")." SET votes=$votes, voters=$voters WHERE poll_id=".$this->getVar("poll_id")."";
		$this->db->query($sql);
	}
}
?>