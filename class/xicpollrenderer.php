...
### =============================================================
### $Id: class/xicpollrenderer.php,v 1.0 2008/01/03 19:00:00 GibaPhp Exp $
### =============================================================
...
class xicpollRenderer
{
	// private
	// xicpoll class object
	var $poll;

	// constructor
	function xicpollRenderer(&$poll)
	{
		$this->poll =& $poll;
	}
	...
	{
        $mydirname = basename( dirname( dirname( __FILE__ ) ) ) ;
		$content = "<form action='".XOOPS_URL."/modules/".$mydirname."/index.php' method='post'>";
		...
		$options_arr =& xicpollOption::getAllByPollId($this->poll->getVar("poll_id"));
		$option_type = "radio";
		...
		foreach ( $options_arr as $option ) {
        	...
        	$content .= "<input type='button' value='"._PL_RESULTS."' class='button' onclick='location=\"".XOOPS_URL."/modules/".$mydirname."/pollresults.php?poll_id=".$this->poll->getVar("poll_id")."\"' />";
    ...
    function assignForm(&$tpl)
    {
		...
		$tpl->assign('poll', array('question'    => $this->poll->getVar("question"), 
								   ...
								   'viewresults' => XOOPS_URL."/modules/".$mydirname."/pollresults.php?poll_id=".$this->poll->getVar("poll_id"), 
								   'action'      => XOOPS_URL."/modules/".$mydirname."/index.php", 'options' => $options));
    }
	...
	function renderResults()
	{
		...
		foreach ( $options_arr as $option ) {
			...
			if ( $percent > 0 ) {
				...
				echo "<img src='".XOOPS_URL."/modules/".$mydirname."/images/colorbars/".$option->getVar("option_color", "E")."' height='14' width='".$width."' align='middle' alt='".intval($percent)." %' />";
			}
		...
		if ( !$this->poll->hasExpired() ) {
			echo "<br />[<a href='".XOOPS_URL."/modules/".$mydirname."/index.php?poll_id=".$this->poll->getVar("poll_id")."'>"._PL_VOTE."</a>]";
		}
    ...
    function assignResults(&$tpl)
    {
		...
		foreach ( $options_arr as $option ) {
			...
			if ( $percent > 0 ) {
				$width = intval($percent)*2;
				$options[$i]['image'] = "<img src='".XOOPS_URL."/modules/".$mydirname."/images/colorbars/".$option->getVar("option_color", "E")."' height='14' width='".$width."' align='middle' alt='".intval($percent)." %' />";
			}
		...
		if ( !$this->poll->hasExpired() ) {
			$vote = "<a href='".XOOPS_URL."/modules/".$mydirname."/index.php?poll_id=".$this->poll->getVar("poll_id")."'>"._PL_VOTE."</a>";
		}
		$tpl->assign('poll', array('question'      => $this->poll->getVar("question"),
		                           'end_text'      => $end_text,
